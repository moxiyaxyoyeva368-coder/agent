...
// Railway production da working dir = /app, development da loyiha root
...
async function finalizePayment(paymentId: string, externalId: string, provider: string) {
  ...
  const txn: PaymentTransaction = {
    id: paymentId, type: pending.type as any, userId: pending.userId,
    userName: user?.name || 'Foydalanuvchi', amount: pending.amount,
    currency: 'UZS', status: 'SUCCESS', paymentMethod: provider as any,
    referenceId: paymentId, description: pending.description,
    createdAt: pending.createdAt, paidAt,
  };
  ...
  if (pending.type === 'SUBSCRIPTION') {
    ...
    const newSub: Subscription = {
      id: `sub-${Date.now()}`, studentId: pending.userId,
      ...
      status: 'ACTIVE', amount: pending.amount,
      paymentMethod: provider, paymentReference: paymentId,
    };
    ...
    notifications.unshift({
      id: `notif-${Date.now()}`, userId: pending.userId, role: 'STUDENT',
      ...
      type: 'SUBSCRIPTION', read: false, createdAt: paidAt,
    });
    ...
    if (idx !== -1) {
      ...
      notifications.unshift({
        ...
        type: 'ARTICLE', read: false, createdAt: paidAt,
      });
    }
  }
...
async function startServer() {
  ...
  app.post('/api/auth/register', async (req, res) => {
    ...
    const newUser: User = {
      id: `user-${Date.now()}`, name, email: email.toLowerCase().trim(),
      role: allowedRoles.includes(role) ? role : 'STUDENT',
      phone, specialty, institution: institution || "Oliy Ta'lim Muassasasi",
      groupId, createdAt: new Date().toISOString(), authProvider: 'local', passwordHash,
    };
    users.push(newUser);
  ...
  app.post('/api/payments/create', requireAuth, (req, res) => {
    ...
    if (!['Click','Payme','Uzum','Stripe'].includes(provider))
    ...
    pendingPayments.set(paymentId, { paymentId, provider, mode, type, userId, amount: Number(amount), orderId, description, createdAt: new Date().toISOString() });
    if (mode === 'redirect') {
      let redirectUrl = '';
      if (provider === 'Click') {
        const p = new URLSearchParams({ service_id: process.env.CLICK_SERVICE_ID||'DEMO', merchant_id: process.env.CLICK_MERCHANT_ID||'DEMO', amount: String(amount), transaction_param: paymentId, return_url: successUrl });
        redirectUrl = 'https://my.click.uz/services/pay?' + p.toString();
      } else if (provider === 'Payme') {
        const encoded = Buffer.from(JSON.stringify({ m: process.env.PAYME_MERCHANT_ID||'DEMO', ac: { order_id: paymentId }, a: amount*100, c: successUrl })).toString('base64');
        redirectUrl = 'https://checkout.paycom.uz/' + encoded;
      } else if (provider === 'Uzum') {
        const p = new URLSearchParams({ merchant_id: process.env.UZUM_MERCHANT_ID||'DEMO', amount: String(amount), order_id: paymentId, return_url: successUrl, cancel_url: failUrl });
        redirectUrl = 'https://checkout.uzumbank.uz/pay?' + p.toString();
      } else if (provider === 'Stripe') {
        redirectUrl = process.env.STRIPE_PUBLISHABLE_KEY
          ? appUrl + '/api/payments/stripe-session?paymentId=' + paymentId
          : appUrl + '/api/payments/stripe-demo?paymentId=' + paymentId + '&amount=' + amount;
      }
    ...
    res.json({ paymentId, status: 'PENDING', mode: 'inline', inlineToken, inlineData: { merchantId: 'DEMO_'+provider, amount: Number(amount), currency: 'UZS', orderId: paymentId, description } });
  });
  ...
  app.post('/api/payments/callback/payme', async (req, res) => {
    ...
    if (method === 'PerformTransaction') {
      await finalizePayment(params?.account?.order_id, String(params?.id), 'Payme');
      res.json({ jsonrpc: '2.0', id, result: { transaction: params?.id, perform_time: Date.now(), state: 2 } });
    } else { res.json({ jsonrpc: '2.0', id, result: {} }); }
  });
  ...
  app.post('/api/payments/callback/stripe', async (req, res) => {
    ...
    if (type === 'checkout.session.completed') await finalizePayment(data?.object?.metadata?.paymentId, data?.object?.id, 'Stripe');
  ...
  app.get('/api/payments/stripe-demo', (req, res) => {
    ...
    const amt = parseFloat(String((req.query as any).amount || '0')).toLocaleString('uz-Latn') + ' UZS';
    const href = '/api/payments/demo-success/' + pid;
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.end('<!DOCTYPE html><html lang="uz"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Stripe Demo</title><style>body{font-family:system-ui,sans-serif;background:#f0f4ff;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0}.c{background:#fff;border-radius:16px;padding:32px;max-width:400px;width:100%;box-shadow:0 8px 32px rgba(0,0,0,.12);text-align:center}h2{color:#1e293b;margin:0 0 6px}p{color:#64748b;font-size:14px}.amt{font-size:28px;font-weight:800;color:#4f46e5;margin:16px 0 20px}input{display:block;width:100%;box-sizing:border-box;padding:10px 12px;border:1.5px solid #e2e8f0;border-radius:8px;font-size:14px;margin-bottom:10px}.row{display:flex;gap:8px;margin-bottom:20px}.row input{margin:0;flex:1}.btn{display:block;width:100%;padding:13px;border-radius:10px;border:none;font-size:15px;font-weight:700;cursor:pointer;margin-bottom:10px}.pay{background:#4f46e5;color:#fff}.cancel{background:#f1f5f9;color:#475569}.note{font-size:11px;color:#94a3b8;margin-top:8px}</style></head><body><div class="c"><div style="font-size:40px">&#128179;</div><h2>Stripe To\'lov (Demo)</h2><p>Demo rejim</p><div class="amt">'+amt+'</div><input placeholder="4242 4242 4242 4242"><div class="row"><input placeholder="MM/YY"><input placeholder="CVC"></div><button class="btn pay" onclick="window.location.href=\''+href+'\'">To\'lovni tasdiqlash</button><button class="btn cancel" onclick="history.back()">Bekor qilish</button><p class="note">&#128274; Demo Stripe</p></div></body></html>');
  });
  ...
  app.post('/api/users', requireAdmin, async (req, res) => {
    ...
    const newUser: User = { id: `user-${Date.now()}`, name, email: email.toLowerCase(), role: role||'STUDENT', groupId, phone, specialty, institution: institution||"Oliy Ta'lim Muassasasi", createdAt: new Date().toISOString(), authProvider: 'local', passwordHash };
    users.push(newUser);
  ...
  // SUBJECTS
  app.get('/api/subjects', requireAuth, (_req, res) => res.json(subjects.map(s => ({ ...s, topicCount: topics.filter(t => t.subjectId === s.id).length }))));
  app.post('/api/subjects', requireAuth, (req, res) => {
    const s: Subject = { id: `subj-${Date.now()}`, name: req.body.name, code: req.body.code||('FAN-'+Math.floor(100+Math.random()*900)), description: req.body.description||'', iconName: req.body.iconName||'BookOpen', category: req.body.category||'Asosiy fanlar', teacherId: req.body.teacherId||'user-teacher-1', teacherName: req.body.teacherName||'Muallim', topicCount: 0, createdAt: new Date().toISOString() };
    subjects.push(s); res.status(201).json(s);
  });
  app.put('/api/subjects/:id', requireAuth, (req, res) => {
    ...
    subjects[i] = { ...subjects[i], ...req.body }; res.json(subjects[i]);
  });
  ...
  app.post('/api/topics', requireAuth, (req, res) => {
    ...
    const t: Topic = { id: `top-${Date.now()}`, subjectId: req.body.subjectId, topicNumber: Number(nextNum), title: req.body.title, description: req.body.description||'', lectureText: req.body.lectureText||'', pdfFileName: req.body.pdfFileName||(nextNum+'-mavzu_maruza.pdf'), pdfFileSize: req.body.pdfFileSize||'2.2 MB', wordFileName: req.body.wordFileName||(nextNum+'-mavzu_maruza.docx'), wordFileSize: req.body.wordFileSize||'750 KB', additionalMaterials: req.body.additionalMaterials||[], passingScore: Number(req.body.passingScore)||settings.defaultPassingScore, questions: req.body.questions||[], createdAt: new Date().toISOString() };
    topics.push(t);
  ...
  app.put('/api/topics/:id', requireAuth, (req, res) => {
    ...
    topics[i] = { ...topics[i], ...req.body }; res.json(topics[i]);
  });
  ...
  app.post('/api/tests/submit', requireAuth, (req, res) => {
    ...
    const att: TestAttempt = { id: `att-${Date.now()}`, studentId, studentName: student?.name||'Talaba', groupId: student?.groupId||'', topicId, subjectId: topic.subjectId, totalQuestions: total, correctAnswers: correct, scorePercentage, passingScore, status, completedAt: new Date().toISOString(), userAnswers };
    attempts.push(att);
  ...
  app.post('/api/subscriptions/purchase', requireAuth, async (req, res) => {
    ...
    const endDate   = new Date(startDate);
    endDate.setMonth(endDate.getMonth() + (settings.subscriptionDurationMonths || 6));
    const newSub: Subscription = { id: `sub-${Date.now()}`, studentId, studentName: student.name, planName: "6 oylik to'liq ta'lim obunasi", durationMonths: settings.subscriptionDurationMonths||6, startDate: startDate.toISOString(), endDate: endDate.toISOString(), status: 'ACTIVE', amount: settings.subscriptionPrice6Months, paymentMethod: paymentMethod||'Click', paymentReference: paymentId };
    subscriptions = subscriptions.map(s => s.studentId===studentId ? { ...s, status: 'EXPIRED' as const } : s);
    subscriptions.unshift(newSub);
    payments.unshift({ id: paymentId, type: 'SUBSCRIPTION', userId: studentId, userName: student.name, amount: settings.subscriptionPrice6Months, currency: 'UZS', status: 'SUCCESS', paymentMethod: (paymentMethod||'Click') as any, referenceId: paymentId, description: "6 oylik obuna", createdAt: new Date().toISOString() });
  ...
  app.post('/api/articles', requireAuth, (req, res) => {
    ...
    const newOrder: ArticleOrder = { id: `art-${orderNum}`, orderNumber: orderNum, studentId, studentName: student?.name||'Talaba', studentEmail: student?.email, studentPhone: student?.phone, title, articleType: articleType||'OAK', volumePages: volumePages||'5-8 bet', language: language||"O'zbek", subjectArea: subjectArea||'Umumiy soha', requirements: requirements||'', deadline: deadline||'7 kun', notes, amount: Number(amount)||settings.articleBasePriceOAK, paymentStatus: 'PAID', paymentMethod: paymentMethod||'Click', status: 'NEW_ORDER', assignedTo: teacher?.id, assignedToName: teacher?.name, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
  ...
  app.put('/api/articles/:id', requireAuth, (req, res) => {
    ...
    if (req.body.uploadedFile || req.body.status === 'READY') notifications.unshift({ id: `notif-${Date.now()}`, userId: prev.studentId, role: 'STUDENT', title: 'Maqolangiz tayyor!', message: '"'+prev.title+'" maqolasi tayyorlandi.', type: 'ARTICLE', read: false, createdAt: new Date().toISOString() });
  ...
  app.get('/api/admin/stats', requireAuth, (_req, res) => {
    ...
    res.json({ totalUsers: users.length, activeUsers: users.filter(u=>u.role==='STUDENT').length, activeSubscriptions: subscriptions.filter(s=>s.status==='ACTIVE').length, expiredSubscriptions: subscriptions.filter(s=>s.status==='EXPIRED').length, totalSubjects: subjects.length, totalTopics: topics.length, totalTests: topics.reduce((s,t)=>s+(t.questions?.length||0),0), totalArticleOrders: articleOrders.length, completedArticles: articleOrders.filter(a=>['READY','COMPLETED','SENT_TO_USER'].includes(a.status)).length, pendingArticles: articleOrders.filter(a=>!['READY','COMPLETED','SENT_TO_USER'].includes(a.status)).length, todayRevenue: todayRev, monthlyRevenue: subRev+artRev, subscriptionRevenue: subRev, articleRevenue: artRev, totalRevenue: subRev+artRev });
  });
  ...
  app.post('/api/ai/generate-quiz', requireAuth, async (req, res) => {
    ...
    res.json({ questions: [{ id: 'q-gen-'+Date.now(), questionText: '"'+topicTitle+'" mavzusining asosiy vazifasi nimadan iborat?', optionA: "Mavzuning nazariy va amaliy metodologiyasini tizimli tadqiq etish", optionB: "Faqatgina tarixiy ma'lumotlarni yod olish", optionC: "Texnik vositalardan cheklangan foydalanish", optionD: "Boshqa fanlar bilan aloqani uzish", correctOption: 'A', explanation: "Har bir akademik fanning tub vazifasi uning metodologiyasini yoritishdir." }] });
  });
